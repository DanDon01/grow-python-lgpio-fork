...
def init_display(invert=False, bgr=False, spi_speed=8000000):
    ...
    try:
        ...
        display.begin()
        logging.info(f"Display initialised: invert={invert}, bgr={bgr}, spi_speed={spi_speed}")
        return display
...
def run_tests():
    ...
    bgr_options = [False, True]
    
    for spi_speed in spi_speeds:
        for invert in invert_options:
            for bgr in bgr_options:
                display = init_display(invert=invert, bgr=bgr, spi_speed=spi_speed)
                if not display:
                    logging.error("Skipping tests due to display initialisation failure")
                    continue

                logging.info(f"Running tests: invert={invert}, bgr={bgr}, spi_speed={spi_speed}")
                for colour_name, colour_value in COLOURS:
                    logging.info(f"Testing colour: {colour_name}")
                    draw_test_pattern(display, colour_value, f"{colour_name} (SPI={spi_speed}, BGR={bgr})")
